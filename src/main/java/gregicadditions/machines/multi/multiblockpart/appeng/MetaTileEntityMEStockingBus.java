package gregicadditions.machines.multi.multiblockpart.appeng;
...
public class MetaTileEntityMEStockingBus extends MetaTileEntityMEInputBus implements IDistinct {
    ...
    @Override
    public void onDistinct(boolean distinct) {
        if (!getWorld().isRemote && !distinct) {
    ...
    private boolean testConfiguredInOtherBus(ItemStack stack) {
        ...
        if (!(controller instanceof GARecipeMapMultiblockController rmmc) || !rmmc.isDistinct()) {
