...
public class GARecipeWrapper implements IRecipeWrapper {
    ...
    public void getIngredients(@Nonnull IIngredients ingredients) {
        ...
        if (!recipe.getOutputs().isEmpty() || !recipe.getChancedOutputs().isEmpty()) {
            ...
            int size = this.recipeMap.getMaxOutputs() + this.recipe.getOutputs().size();
            for (int i = 0; i < chancedOutputs.size(); i++) {
                ...
                this.chanceEntries.put(i + size, ArrayUtils.addAll(new int[0], chancedEntry.getChance(), chancedEntry.getBoostPerTier()));
            }
    ...
    public static String getMinTierForVoltage(long voltage) {
        for (int i = 0; i < GAValues.V.length; ++i) {
            if (voltage <= GAValues.V[i]) {
                return GAValues.VN[i];
            }
        }
        return "";
    }
